...
%                                          1 = notify imports)
%==========================================================================
function result = ReadYaml(filename, verbose)
    if ~exist('verbose','var')
        verbose = 0;
    end;
    
    [pth,~,~,~] = fileparts(mfilename('fullpath'));       
    javaaddpath([pth filesep 'external' filesep 'snakeyaml-1.8.jar']); % javaaddpath clears global variables...!?
...
function result = scan_map(r)
    ...
    while it.hasNext()
        ...
        ich = char(i);
        if iskw_import(ich)
            result.(ich) = perform_import(r.get(i));
        else
            result.(ich) = scan(r.get(i));
        end;
    end;
end
